...
public class S_ListeningMaster {

	// Listening's Frame
	private JFrame frmListeningMaster;
	...
	// Editing data
	private ListeningDto editingListeningDto;

	// Edit mode
	private EditMode editMode;

	/**
	 * Create Listening screen.
	 *
	 * @param frmParent
	 */
	public S_ListeningMaster(JFrame frmParent) {
	...
	private void initComponents() {
		...
		frmListeningMaster.getContentPane().add(panel);
	...
	private void showData() {
		listeningDtoList = new ArrayList<ListeningDto>();
		deletedList = new ArrayList<Long>();

		// Search all Listening
		ComboItem lessonItem = (ComboItem) cbxLesson.getSelectedItem();
		listeningDtoList = ListeningService.searchListening(lessonItem.getValue());
	...
	private void actMoveRow(MoveType moveType) {
		...
		if (selectedIndex < 0) {
			MessageUtil.showInfoMessage(frmListeningMaster, Constants.MSG_NOT_CHOOSE_ROW);
			return;
		}
		...
		if (selectedIndex == (listeningDtoList.size() - 1)
		...
		// Get selected row
		ListeningDto listeningDto = listeningDtoList.get(selectedIndex).copy();

		// Remove old row
		listeningDtoList.remove(selectedIndex);
		...
		if (MoveType.MOVE_TOP.equals(moveType)) {
			...
			selectedIndex = listeningDtoList.size();
		}
		listeningDtoList.add(selectedIndex, listeningDto);
	...
	private void actNew() {
		// Initial edit data
		editingListeningDto = new ListeningDto();
	...
	private void actDelete() {
		...
		if (MessageUtil.showConfirmMessage(frmListeningMaster, Constants.MSG_DELETE_CONFIRM)) {
			...
			Long listeningId = listeningDtoList.get(selectedIndex).getListeningId();
			if (listeningId != null) {
				deletedList.add(listeningId);
			}
	...
	private void actRegister() {
		// Show confirm message
		if (MessageUtil.showConfirmMessage(frmListeningMaster, Constants.MSG_REGISTER_CONFIRM)) {

			// Get selected lesson
			ComboItem lessonItem = (ComboItem) cbxLesson.getSelectedItem();

			// Register all Listening
			if (!ListeningService.registerAll(listeningDtoList,
											   deletedList,
											   lessonItem.getValue())) {
				MessageUtil.showErrorMessage(frmListeningMaster, Constants.MSG_REGISTER_FAIL);
				return;
			}

			// Show success message
			MessageUtil.showInfoMessage(frmListeningMaster, Constants.MSG_REGISTER_SUCCESS);
	...
	private void actBack() {
		// Show confirm message
		if (MessageUtil.showConfirmMessage(frmListeningMaster, Constants.MSG_CANCEL_CONFIRM)) {
			frmListeningMaster.dispose();
	...
	private void actOK() {
		...
		if (EditMode.NEW.equals(editMode)) {
			// Mode New
			listeningDtoList.add(editingListeningDto);
		} else {
			// Mode Modify
			selectedIndex = tblInfo.getSelectedRow();
			listeningDtoList.set(selectedIndex, editingListeningDto);
		}
	...
	private Boolean validateRegisterData() {
		// Content File
		if (StringUtil.isNullOrEmpty(txtContentFile.getText())) {
			MessageUtil.showErrorMessage(frmListeningMaster, "Please input [Content File].");
			return false;
		}
	...
	/**
	 * @return the frmListeningMaster
	 */
	public JFrame getFrame() {
		return frmListeningMaster;
	}
}